...
`done` (shipped in a named slice — kept here for the audit trail).

| Item | Status | Target | Why not v0 | Pointer |
|---|---|---|---|---|
| **SMS / phone OTP auth rail (India +91)** | `feature-flag` | Turn on when MSG91 + DLT are ready | **Decided 2026-08-29 (S-005, ADR-003):** optional sign-in method, phone-only accounts allowed, MSG91 provider. Code path exists behind `AUTH_PHONE_OTP` (backend) + `NEXT_PUBLIC_AUTH_PHONE_OTP` (web), both default `false` and off in CI. Frozen spec untouched. Supabase Auth does the SMS send; FastAPI still only verifies the JWT. | [`ADR-003`](agents/adrs/ADR-003-optional-phone-otp-login.md); [`S-005`](agents/slices/S-005-phone-otp-login.md); `mvp-spec.md` §3.1, §13.1 |
| Phone / email account linking (same person, two `sub`s) | `deferred` | post-v0 | With phone OTP optional, a user who signs up once by email and once by phone gets two Supabase users → two local rows. v0 does not merge them. | [`ADR-003`](agents/adrs/ADR-003-optional-phone-otp-login.md) |
...
| Razorpay Checkout.js integration + `/auth/callback` | `done` | S-006 (Accepted 2026-08-30) | S-004 wired `checkout-session` and opens `window.Razorpay` when `checkout.js` + a real key happen to be present. S-006 loads the hosted Checkout.js `<script>` on `/billing` via `next/script`, adds the success / dismiss / `payment.failed` handlers and the mock-key / no-script fallback, and adds the Next.js `/auth/callback` code-exchange route (ADR-004) that every sign-in path funnels through. No new `/v1` endpoint, no LLM, no quota. The real Razorpay modal (M-004) and real Supabase OAuth round-trip (M-005) stay pre-launch manual checks. | `docs/agents/slices/S-006-hosted-checkout.md`; `docs/agents/adrs/ADR-004-auth-redirect-callback.md` |
| Deep-link-back-after-login (same-origin `next` on `/auth/callback`) | `deferred` | post-v0 | S-006 / ADR-004 fixed `/auth/callback` to redirect to `/` unconditionally — no `next` / `redirectTo` param is read. A same-origin, open-redirect-guarded `next` is the follow-up if a real deep-link-back need appears. | `docs/agents/adrs/ADR-004-auth-redirect-callback.md`; `docs/agents/slices/S-006-hosted-checkout.md` |
| Near-real-time plan reflection after payment (bounded poll / SSE) | `deferred` | post-v0 | S-006 client success is a single `GET /v1/me` refetch — the plan flips only once the webhook has run and the user reloads `/billing`. A bounded poll or a server push would reflect the upgrade within seconds of payment. | `docs/agents/slices/S-006-hosted-checkout.md` (Risks) |
| `GET /v1/plans` SKU catalog endpoint | `idea` | TBD | The `/billing` "₹500/mo" card label is a client constant; the *charged* amount is always server-authoritative from `checkout-session`. A read-only plans endpoint would let the client render the catalog without a hard-coded label. | `docs/agents/slices/S-006-hosted-checkout.md` (Risks); `mvp-spec.md` §5.1 |
